...
static CHAR *myStrcat(CHAR *dest, CHAR *name) {
    while ( *name )
        *dest++ = *name++;
    *dest = '\0';
    return dest;
...
BOOL ProcessEnumerate()
{
    ...
    BitNames protectionNames[] = {
        ...
        { PAGE_NOACCESS,          "PAGE_NOACCESS"          }, // seems to be used with other bits ??
        { 0, NULL },
        // never reach print for guard, or nocache pages
    };

    Syelog(SYELOG_SEVERITY_INFORMATION,
           "### Initial process virtual memory\n");
    ...
    for (PBYTE pbRegion = (PBYTE)0x10000;; pbRegion = pbNext) {
        ...
        if (pinh &&
            ...
            Syelog(SYELOG_SEVERITY_INFORMATION, "###   %8p..%8p: %ls\n", pbRegion, pbNext, wzDllName);
        }
        else {
            CHAR bitNames[4096], *p;
            BitNamesP bp;
            DWORD bits;
            if ( mbi.Protect ) {
                for ( bits = mbi.Protect, bp = protectionNames, p = bitNames ; bp->name != NULL ; bp++ ) {
                    if ( bits & bp->bit ) {
                        if ( p != bitNames )
                            *p++ = '|';
                        p = myStrcat(p, bp->name);
                        bits &= ~bp->bit;
                    }
                }
                *p++ = '\0';
                if ( bits == 0 ) {
                    Syelog(SYELOG_SEVERITY_INFORMATION, "###   %8p..%8p: Protect = %s\n", pbRegion, pbNext, bitNames);
                } else {
                    Syelog(SYELOG_SEVERITY_INFORMATION, "###   %8p..%8p: Protect = %s|0x%x\n", pbRegion, pbNext, bitNames, bits);
                }
            }

        }
        if ( mbi.State != MEM_COMMIT )
            Syelog(SYELOG_SEVERITY_INFORMATION, "###     State   = 0x%x\n", mbi.State);

    }
    Syelog(SYELOG_SEVERITY_INFORMATION, "###\n");

    LPVOID lpvEnv = Real_GetEnvironmentStringsW();
    Syelog(SYELOG_SEVERITY_INFORMATION, "### Initial environment %08x\n", lpvEnv);
    PWCHAR pEnv = (PWCHAR) lpvEnv;
    while ( *pEnv ) {
